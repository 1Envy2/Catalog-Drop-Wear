  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] py-20 bg-white border border-gray-100">
        ...
        <h2 className="text-3xl font-medium tracking-tighter text-[#111111] mb-2 uppercase">
          Your bag is empty
        </h2>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 pb-20">
      <div className="lg:col-span-8">
        <h1 className="text-4xl font-medium tracking-tighter uppercase mb-12 border-b border-gray-100 pb-8">
        ...
        <div className="divide-y divide-gray-100">
          {items.map((item) => {
            ...
            // PERBAIKAN: Pastikan size dan color dikonversi dari null ke undefined
                ...
                <div className="flex-1 flex flex-col justify-between">
                  ...
                  <div className="flex items-center justify-between mt-auto pt-6">
                    <div className="flex items-center border border-gray-100 h-10 px-2">
                      <button
                        onClick={() => updateQuantity(item.product_id, item.quantity - 1, currentSize, currentColor)}
                        className="p-2 hover:text-[#E2FF3B]"
                      >
                        <Minus size={14} />
                      </button>
                    ...
                    <button
                      onClick={() => removeItem(item.product_id, currentSize, currentColor)}
                      className="text-xs font-bold uppercase tracking-widest text-gray-300 hover:text-red-500 transition-colors"
                    >
                      Remove
                    </button>
                  </div>
      ...
      <div className="lg:col-span-4">
        <div className="bg-[#F9F9F9] p-10 sticky top-28 border border-gray-100">
          ...
          <div className="space-y-6 text-sm border-b border-gray-200 pb-8 mb-8">
            ...
            <div className="flex justify-between text-gray-500">
              <span>Shipping</span>
              <span className="text-[#111111] font-bold">Calculated at next step</span>
            </div>
          </div>
          ...
          <Link href="/checkout">
            <Button className="w-full bg-[#111111] text-white rounded-none h-14 font-bold uppercase tracking-widest text-xs hover:bg-[#E2FF3B] hover:text-[#111111] mb-4">
              Checkout Now <ArrowRight size={16} className="ml-2" />
            </Button>
          </Link>
          <button
            onClick={() => clearCart()}
            className="w-full text-[10px] font-bold uppercase tracking-widest text-gray-400 hover:text-[#111111] transition-colors py-2"
          >
            Clear Shopping Bag
          </button>
        </div>
